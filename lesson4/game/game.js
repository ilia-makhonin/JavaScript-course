...
var stroke = [0];  //Создал массив для хранения результатов
...
function guessNumber(){
	...
	while(gameIsRunning){
		...
		// проверяем число
			...
			stroke.push(result);                    //Добавляю в массив результат каждой попытки
			...
			if(answer[0]){
				...
				while (str) {                        //Просмотр ходов
					var user_answer = parseInt(prompt("Вы можете посмотреть свои ходы. \n " +
						"Введите номер хода. Для выхода введите -2"));

					if (user_answer == -2) {
						str = false;
					} else {
						alert(stroke[user_answer]);
					}
				}
				// останавливаем игру
				gameIsRunning = false;
